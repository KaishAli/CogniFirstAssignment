...
const Weatherdetails = () => {
  ...
  const getweather = (capital: string) => {
    fetch(
      `http://api.weatherstack.com/current?access_key=d81381e7f65ac20a3115570356e0a315&query=${capital}`
    )
      .then((res) => res.json())
      .then((data) => {
        setweather({
  ...
  return (
    <div>
      <h3 style={{ textAlign: "center", marginBottom: "50px" }}>
        {" "}
        Current Weather Condition of {weatherdata.capital}
      </h3>
